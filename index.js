  ...
  .setResponseHandler('GetInventory', function (data) {
    if (data.success) {
      ...
      var candy = _.reduce(delta.inventory_items, function (result, item) {
        // this is unnecessarily complicated because I had a bugged pidgey entry
        // in conclusion: pidgey is an asshole
        return (item.inventory_item_data.pokemon_family !== undefined &&
               !item.inventory_item_data.pokemon_family.inventory_item_data) ? _.concat(result, item.inventory_item_data.pokemon_family) : result;
      });
      if (inventory === null) {
